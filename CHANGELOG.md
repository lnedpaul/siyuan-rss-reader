...
- **内置订阅源大幅扩充**: `featured-feeds.json` 从 7 个源扩展至 31 个，新增 4 个分类
  - **新闻** (10个): 中新网 8 分类（实时/要闻/国内/国际/社会/体育/文化/军事）+ IT之家 + 澳门文化局
  - **财经** (6个): 中新网财经/证券/财富/能源/房地产 + 东方财富
  - **天气** (5个): 中国气象局重要天气/气象要闻/媒体聚焦/工作动态/气象科技
  - **学术期刊** (3个): arXiv CS / PNAS / Nature

## [0.1.22] - 2026-05-29

### 改进

- **按钮颜色系统重构**: 全面迁移为 HSL 颜色体系，统一 `--hue-*` 变量，深浅主题仅变化明度/饱和度
- **角标实时更新**: 单篇滚动阅读时角标实时递减，不再等待全量重渲
  - 添加 `updateBadgeDOM` 方法直接操作 DOM 更新角标
  - 刷新、后台取回后始终更新角标，即使已切换订阅
  - 所有刷新路径（`updateUIAfterRefresh`、`fetchArticle` 后台取回、`refreshAllFeeds`）均已覆盖
- **角标上限显示**: 超过 99 条未读时显示 `99+`，避免角标数字溢出
- **后台检查稳定性**: 设置保存后不再丢失默认 30 分钟后台检查定时器
- **缓存清理同步**: 每日缓存清理后自动刷新角标计数

### 修复

- **按钮动画抖动**: 所有按钮宽高过渡统一使用 `cubic-bezier(0.25, 0.8, 0.25, 1)`，消除 hover 动画抖动
- **垂直/水平布局翻转**: 修正 `isH = this.settings.layout === 'vertical'` 语义，垂直布局按垂直分隔线排版
- **vResizer 条件不一致**: mousedown 和 onMove 中 layout 判断统一为 `'vertical'`
- **JSON 语法错误**: 移除 `clearCache` 后 i18n 末尾残留逗号导致所有标签显示 `undefined`
- **添加按钮卡死**: hover 宽度从被锁定的 `30px` 修正为 `100px`
- **刷新后角标虚高**: `updateUIAfterRefresh` 漏查 `this.readStatus` 导致已读文章被重复计数
- **定时器丢失**: `setupAutoRefresh` 覆盖默认 30 分钟检查定时器，用户保存设置后后台检查永久失效
- **死代码清理**: 移除未实现的 `showUnreadOnly` 配置项及相关 i18n
